...
export default function ProductsPage() {
    ...
    const handleCreateProduct = async (e: React.FormEvent) => {
        ...
        if (!selectedSubcategoryId) {
            setFormError('Por favor selecciona una subcategoría para el producto.');
            return;
        }
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-slate-800 text-xs font-semibold text-slate-400 uppercase tracking-wider bg-slate-900/50">
                                    ...
                                    <th className="py-4 px-6">Subcategoría</th>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full shadow-2xl overflow-hidden">
                        ...
                        <form onSubmit={handleCreateProduct} className="p-6 space-y-4">
                            ...
                            <div>
                                <label className="text-xs font-semibold text-slate-300 uppercase tracking-wider">Categoría Padre</label>
                                <select
                                    required
                            ...
                            <div>
                                <label className="text-xs font-semibold text-slate-300 uppercase tracking-wider">Subcategoría</label>
                                <select
                                    required
                                    disabled={!selectedCategoryId}
                                    value={selectedSubcategoryId}
                                    onChange={(e) => setSelectedSubcategoryId(e.target.value)}
                                    className="mt-2 block w-full rounded-lg border border-slate-800 bg-slate-950 px-3 py-2.5 text-white placeholder-slate-600 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 sm:text-sm disabled:opacity-50"
                                >
                                    <option value="" disabled className="bg-slate-950 text-slate-500">
                                        {selectedCategoryId ? 'Seleccionar subcategoría...' : 'Primero elige una categoría'}
                                    </option>
